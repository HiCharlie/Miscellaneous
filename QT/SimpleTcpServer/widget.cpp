...
void Widget::initServer()
{
    ...
    connect(ui->btnListen,&QPushButton::clicked,[this]{
        ...
        if(server->isListening()){
            ...
            //从界面上读取ip和端口
            //可以使用QHostAddress::Any 监听所有地址的所有端口
    ...
    connect(server,&QTcpServer::newConnection,this,[this]{
        ...
        while(server->hasPendingConnections()){
            ...
            connect(socket, static_cast<void(QAbstractSocket::*)(QAbstractSocket::SocketError)>(&QAbstractSocket::error),
                ...
                ui->editRecv->append(QString("[%1:%2] Soket Error:%3")
                                     .arg(socket->peerAddress().toString())
                                     .arg(socket->peerPort())
            ...
            connect(socket,&QTcpSocket::disconnected,[this,socket]{
                ...
                ui->editRecv->append(QString("[%1:%2] Soket Disonnected")
                                     .arg(socket->peerAddress().toString())
                                     .arg(socket->peerPort()));
                updateState();
            });
    ...
    connect(server,&QTcpServer::acceptError,[this](QAbstractSocket::SocketError){
        ui->editRecv->append("Server Error:"+server->errorString());
    });
}
...
{
    //当前server地址和端口、客户端连接数写入标题栏
    if(server->isListening()){
        setWindowTitle(QString("Tcp Server[%1:%2] connections:%3")
